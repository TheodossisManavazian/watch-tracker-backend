...
def _update_watch_price_data(watch: WatchFull, market_price: float) -> dict:
    price_change = market_price - watch.pricing["retail_price"]
    percent_change = price_change / watch.pricing["retail_price"]
...
def update_watch_market_price():
    with engine.connect() as conn:
        ...
        for w in watches:
            ...
            if ls:
                listings = [Listing.from_db(l) for l in ls]
                market_price = _calculate_market_price(listings)
                _update_watch_price_data(watch, market_price)
                watch_service.upsert_watch_pricing(conn, dict(watch))
